  ...
  <table class="textcenter">
    ...
    <tr>
      ...
      <td class="{if $result.status eq $searchvar}textgreen{else}textred{/if}">
        <label>{if $result.status eq $searchvar}<input type="checkbox" name="domains[]" value="{$result.domain}"{if $num eq 0} checked{/if} />
        {$LANG.domainavailable}{else}{$LANG.domainunavailable}{/if}</label></td>
