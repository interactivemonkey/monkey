...
<tr class="rowcolor1"><td colspan="2"><label><input type="radio" name="custtype" value="new" id="custtypenew" onclick="showsignupfields()" {if $loggedin}disabled{else}checked{/if} /> I'm a New Customer</label> <label><input type="radio" name="custtype" value="existing" id="custtypeexisting" {if $loggedin}checked{else}onclick="showloginfields()"{/if} /> I'm an Existing Customer</label></td></tr>
...
<tr class="rowcolor2 newccinfo"{if $clientsdetails.cclastfour && $ccinfo neq "new"} style="display:none;"{/if}><td class="fieldarea">{$LANG.creditcardcardexpires}</td><td><select name="ccexpirymonth" id="ccexpirymonth" class="newccinfo">{foreach from=$months item=month}<option>{$month}</option>{/foreach}</select> / <select name="ccexpiryyear" class="newccinfo">{foreach from=$years item=year}<option>{$year}</option>{/foreach}</select></td></tr>
{if $showccissuestart}
<tr class="rowcolor1 newccinfo"{if $clientsdetails.cclastfour && $ccinfo neq "new"} style="display:none;"{/if}><td class="fieldarea">{$LANG.creditcardcardstart}</td><td><input type="text" name="ccstartmonth" size="2" maxlength="2" value="{$ccstartmonth}">/<input type="text" name="ccstartyear" size="2" maxlength="2" value="{$ccstartyear}" /> (MM/YY)</td></tr>
...
<p align="center"><input type="submit" value="{$LANG.completeorder}"{if $cartitems==0} disabled{/if} /></p>
