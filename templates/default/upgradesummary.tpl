...
{if $promorecurring}
<div class="alert-message info textcenter">
...
{$promocode} - {$promodesc} <input type="submit" name="removepromo" value="{$LANG.orderdontusepromo}" class="btn error" />
{else}
...
<p>{foreach key=num item=gateway from=$gateways}<label style="width:auto;padding-right:10px;"><input type="radio" name="paymentmethod" value="{$gateway.sysname}" {if $selectedgateway eq $gateway.sysname} checked{/if}> {$gateway.name} </label>{/foreach}</p>
