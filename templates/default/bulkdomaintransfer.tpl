...
<div class="well">
    ...
    <div class="textcenter">
        ...
        {/if}
        <div class="internalpadding"><input type="submit" value="{$LANG.domainstransfer}" class="btn success large" /></div>
    </div>
</div>
...
{foreach from=$availabilityresults key=num item=result}
        <tr>
            ...
            <td class="textcenter">{if $result.status eq "unavailable"}<select name="domainsregperiod[{$result.domain}]">{foreach key=period item=regoption from=$result.regoptions}<option value="{$period}">{$period} {$LANG.orderyears} @ {$regoption.register}</option>{/foreach}</select>{/if}</td>
        </tr>
{/foreach}
</table>
