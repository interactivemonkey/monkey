...
<p class="heading2">{$LANG.clientareanavcontacts}</p>

{if $contactid}

<form method="post" action="{$smarty.server.PHP_SELF}?action=contacts">
<p>{$LANG.clientareachoosecontact}: <select name="id" onchange="submit()">{foreach item=contact from=$contacts}
...
<tr class="subaccountfields"{if !$subaccount} style="display:none;"{/if}><td class="fieldarea">{$LANG.subaccountpermissions}</td><td><input type="checkbox" name="permissions[]" id="permprofile" value="profile"{if in_array('profile',$permissions)} checked{/if} /> <label for="permprofile">{$LANG.subaccountpermsprofile}</label><br />
...
<input type="checkbox" name="permissions[]" id="permorders" value="orders"{if in_array('orders',$permissions)} checked{/if} /> <label for="permorders">{$LANG.subaccountpermsorders}</label></td></tr>
...
<p align="center"><input type="submit" value="{$LANG.clientareasavechanges}" class="buttongo" /> <input type="reset" value="{$LANG.clientareacancel}" class="button" /> <input type="button" value="{$LANG.clientareadeletecontact}" class="buttonwarn" onclick="deleteContact()" /></p>

</form>

{else}

<div class="errorbox">&nbsp;#&nbsp {$LANG.clientareanocontacts} &nbsp;#&nbsp;</div>
