<p class="heading2">{$LANG.domainrenew}</p>

<p>{$LANG.domainrenewdesc}</p>

<p align="center"><strong>{$domain}</strong><br />{$LANG.domaincurrentrenewaldate}: {$nextduedate}</p>

<form method="post" action="{$smarty.server.PHP_SELF}?action=domainrenew">
<input type="hidden" name="domainid" value="{$domainid}" />
<input type="hidden" name="renew" value="true" />

<p align="center"><select name="renewalperiod">
...
{/foreach}
</select> <input type="submit" value="{$LANG.domainorderrenew}" class="buttongo" /></p>
