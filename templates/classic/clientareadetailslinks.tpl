...
{if $condlinks.updatesq} | {if $clientareaaction eq "changesq"}<strong>{$LANG.clientareanavsecurityquestions}</strong>{else}<a href="{$smarty.server.PHP_SELF}?action=changesq">{$LANG.clientareanavsecurityquestions}</a>{/if}{/if}
</div>